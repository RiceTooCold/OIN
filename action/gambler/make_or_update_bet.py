...
class handle_bet_transaction(Action):
    ...
    def exec(self, conn, db, cur, user):
        try:
            ...
            conn.send("---Start make or update your own bets---\n\n".encode('utf-8'))
            ...
            while True:
                conn.send("[INPUT]Enter the Record ID: ".encode('utf-8'))
            ...
            if bet_exist_or_not:
                ...
                conn.send("\n[INPUT]Do you want to add a new bet or cancel an existing one? (a/c): ".encode('utf-8'))
                action = conn.recv(1024).decode('utf-8').strip().lower()
                if action == "c":
                    conn.send("[INPUT]Enter the number of the bet you want to cancel: ".encode('utf-8'))
            ...
            if bet_exist_or_not == 0 or action == "a":
                # Add new bet
                while True:
                    conn.send("[INPUT]Choose which side to bet on (Home/Away): ".encode('utf-8'))
                ...
                while True:
                    conn.send("[INPUT]Enter the amount to bet: ".encode('utf-8'))
                    try:
                ...
                conn.sendall(f"Bet successfully added at {new_bet_time}.\n".encode('utf-8'))
                conn.sendall(f"Transaction ID: {trans_id}\nSelected Odd: {selected_odd}\nNew balance: {new_balance}\n".encode('utf-8'))
