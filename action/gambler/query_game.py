...
class query_game(Action):
    ...
    def fetch_game_details(self, game_id, conn, db, cur):
        ...
        try:
            ...
            if history_data:
                ...
                data_table = print_table(history_data, cur)    
    ...
    def gambling_phase(self, bet_types, conn, db, cur):
        ...
        while True:
            conn.send("\n[INPUT]Do you want to enter the gambling phase? Enter the bet type number or 0 to exit.\n".encode('utf-8'))
            conn.send("Your choice: ".encode('utf-8'))
    ...
    def exec(self, conn, db, cur, user):
        conn.send("[INPUT]Enter the Game ID to fetch details: ".encode('utf-8'))
